...
# ReAct の 3～4 回のツール利用を許容するようデフォルト12ステップを確保しておく。
DEFAULT_RECURSION_LIMIT = 12

OFFLINE_SEARCH_DATA = {
    "answer": (
        "2024年の生成AI業界では導入率が 33% から 71% へ拡大し、"
        "顧客エンゲージメントやコスト管理、製品開発での活用が急増しています。"
        "ROI を得た企業はカスタムアプリケーション投資を加速しました。"
    ),
    "results": [
        {
            "url": "https://www.mckinsey.com/capabilities/quantumblack/our-insights/the-state-of-ai",
            "title": "The State of AI: Global survey",
            "content": "Use of generative AI increased from 33% in 2023 71% in 2024. Use of AI by business function ...",
        },
        {
...
def _content_to_text(content: object) -> str:
    """LangChain Message.content 文字列へ正規化する"""
...
def _extract_result_count(payload: object) -> int:
    """TolMessage payload から results 配列の長さを推定する(LangGraph v1互換)。"""
...
def compute_summary_stats(result: dict, final_answer: str) -> tuple[int, int, int]:
    ...
    # LangGraph create_agent は intermedate_steps を返さないため、messages から推定する。
...
def main(
    ...
    question = query or "2026年の生成 AI 業界の主要な動向を3つ教えてください"
